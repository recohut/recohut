...
__all__ = ['random_split', 'stratified_split', 'split_by_ratio', 'split_random_holdout', 'split_temporal_holdout',
           'split_last_session_out', 'random_split_v2', 'chrono_split', 'TimeRangeSplit', 'stratified_split_v2',
           'split_interaction_matrix', 'leave_one_out_split']
...
# Cell
def split_random_holdout(dataset, perc=0.8, seed=1234):
    """
...
# Internal Cell
def _clean_split(train, test, seq_col='sequence'):
    """
    ...
    """
    train_items = set()
    train[seq_col].apply(lambda seq: train_items.update(set(seq)))
    test[seq_col] = test[seq_col].apply(lambda seq: [it for it in seq if it in train_items])
    return train, test

# Cell
def split_temporal_holdout(dataset, ts_threshold, ts_col='timestamp', seq_col='sequence'):
    """
    ...
    :return: the training and test splits
    """
    train = dataset.loc[dataset[ts_col] < ts_threshold]
    test = dataset.loc[dataset[ts_col] >= ts_threshold]
    train, test = _clean_split(train, test, seq_col=seq_col)

    return train, test

# Cell
def split_last_session_out(data,
                           user_col='user_id',
                           sess_col='session_id',
                           seq_col='sequence',
                           time_col='ts'):
    ...
    """
    sessions = data.sort_values(by=[user_col, time_col]).groupby(user_col)[sess_col]
...
# Cell
def split_interaction_matrix(matrix: np.ndarray,
                             num_test: int = 10,
                             sparse: bool = True) -> Tuple[np.ndarray, np.ndarray]:
    """
    Split the numpy array interaction matrix into train/test

    Args:
        matrix: numpy array of user/item interactions
        num_test: number of items to select / row to place into test
        sparse: returns sparse matrices if True
    """
    test = np.zeros(matrix.shape)
    train = matrix.copy()
    for user in range(matrix.shape[0]):
        if matrix[user, :].nonzero()[0].shape[0] > num_test:
            test_interactions = np.random.choice(matrix[user, :].nonzero()[0],
                                                 size=num_test,
                                                 replace=False)
            train[user, test_interactions] = 0.
            test[user, test_interactions] = matrix[user, test_interactions]
...
def leave_one_out_split(df: pd.DataFrame,
    ...
    _df = df.copy()
    _df['rank_latest'] = _df.groupby([user_col])[ts_col].rank(method='first', ascending=False)
    train_df = _df[_df['rank_latest'] != 1]
    test_df = _df[_df['rank_latest'] == 1]
    train_df.drop('rank_latest', axis=1, inplace=True)
    test_df.drop('rank_latest', axis=1, inplace=True)
